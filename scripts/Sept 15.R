...
fishing <- read_excel("data/raw/commercial.xls", sheet = "Erie")
...
fishing_long <- fishing |> 
  pivot_longer(
    names_to = "region",
    values_to = "values",
    cols = !c(Year, Lake, Species, Comments) # what does this mean?
  )
...
# Are all seven of these the same kind of thing? What's the grain? 
